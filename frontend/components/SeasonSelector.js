...
export default function SeasonSelector({ onSeasonChange, currentSeasonId }) {
  ...
  if (loading) {
    return (
      <div className="w-64 h-12 bg-neutral-200 animate-pulse rounded-lg"></div>
    );
  }
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center justify-between w-64 px-4 py-3 bg-white/90 border border-neutral-200 rounded-lg hover:bg-white transition-all shadow-sm"
      >
      ...
      {isOpen && (
        <div className="absolute top-full left-0 w-full mt-2 bg-white border border-neutral-200 rounded-lg shadow-lg z-50 max-h-64 overflow-y-auto">
          {seasons.map((season) => (
            <button
              key={season.id}
              ...
              className={`w-full px-4 py-3 text-left hover:bg-neutral-50 transition-colors border-b border-neutral-100 last:border-b-0 ${
                selectedSeason?.id === season.id ? 'bg-primary-50 text-primary-700' : 'text-neutral-700'
              }`}
            >
